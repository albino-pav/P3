...
private:
  ...
  unsigned int frameLen, ///< length of frame (in samples). Has to be set in the
                         ///< constructor call
      samplingFreq, ///< sampling rate (in samples per second). Has to be set in
  ...
  ///
  void autocorrelation(const std::vector<float> &x,
...
public:
  PitchAnalyzer(
      ...
      float min_F0 =
          MIN_F0, ///< Pitch range should be restricted to be above this value
      float max_F0 =
          MAX_F0 ///< Pitch range should be restricted to be below this value
  ) {
    frameLen = fLen;
    samplingFreq = sFreq;
  ...
  ///
  /// Operator (): computes the pitch for the given vector, expressed by the
  ...
  ///
  /// Sets pitch range: takes min_F0 and max_F0 in Hz, sets npitch_min and
  /// npitch_max in samples
  ///
